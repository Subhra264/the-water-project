  ...
  const navBarLinks = () => {
    if (!userState) {
      return [
        ...
        <Link to="/sign-up" key="sign-up">
          Sign Up
        </Link>,
      ];
    } else {
      return [
        <Link to={`/discussion/users/${userState.id}/`} key="profile">
          Profile
        </Link>,
        <Link to="/discussion/create-ngo/" key="create-ngo">
          Create NGO
        </Link>,
        <button onClick={logOut} key="log-out">
          Log out
        </button>,
      ];
    }
